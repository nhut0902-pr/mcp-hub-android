...
export function VideoScreenHeader({ title }: { title: string }) {
  const router = useRouter();
  return <View style={styles.header}><Pressable onPress={() => router.replace("/settings")} style={styles.back} hitSlop={10}><MaterialIcons name="arrow-back-ios-new" color={palette.text} size={18} /></Pressable><Text style={styles.title}>{title}</Text><View style={styles.spacer} pointerEvents="none" /></View>;
}
