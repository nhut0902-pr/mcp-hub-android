...
export function providerLogoUrl(kind: ProviderKind, name: string): string | null { return brandLogoUrl(providerSlugs[kind] ?? mcpSlugs.find(([pattern]) => pattern.test(name))?.[1]); }
