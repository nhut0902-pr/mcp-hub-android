...
export default function ChatScreen() {
  ...
  const generateImageInChat = async () => {
    ...
    setDraft("");
    setImageMode(false);
    const userMsg: ChatMessage = { id: `msg_${Date.now()}`, role: "user", content: `🎨 Tạo ảnh: ${prompt}`, attachments: [], reasoning: undefined, failure: undefined };
    setMessages((prev) => [...prev, userMsg]);
    try {
      const url = generateImageUrl(prompt, { width: 768, height: 768, model: "flux" });
      const aiMsg: ChatMessage = { id: `msg_${Date.now() + 1}`, role: "assistant", content: `Đã tạo ảnh cho: "${prompt}"`, attachments: [{ id: `img_${Date.now()}`, type: "image", uri: url, label: prompt }], reasoning: undefined, failure: undefined };
      setMessages((prev) => [...prev, aiMsg]);
    } catch (e) {
      const errMsg: ChatMessage = { id: `msg_${Date.now() + 1}`, role: "assistant", content: "Không thể tạo ảnh. Thử lại sau.", attachments: [], reasoning: undefined, failure: undefined };
...
const styles = StyleSheet.create({
  ...
  imageModeBar: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, paddingVertical: 8, marginHorizontal: 10, marginBottom: 4, borderRadius: 10, backgroundColor: 'rgba(10,132,255,0.1)', borderWidth: 1, borderColor: 'rgba(10,132,255,0.2)' },
  imageModeText: { flex: 1, color: palette.primary, fontSize: 12, fontWeight: '600' },
  chatList: { flex: 1 },
