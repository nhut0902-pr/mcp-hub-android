...
import { appendMcpToolResults, extractMcpToolCalls, mcpCallMayChangeExternalData, summarizeMcpResults, toOpenAiMcpTools, type McpProposedCall } from "@/lib/mcp-hub/mcp-chat";
...
export default function ChatScreen() {
  ...
  const send = async () => {
    ...
    try { const basePayload = buildChatPayload(provider, model, history, { thinking, webSearch, temperature: tuning.temperature, maxTokens: tuning.maxTokens, topP: tuning.topP, instruction: tuning.instruction }); const mcpTools = await loadEnabledMcpTools(); const payload = mcpTools.length ? { ...basePayload, tools: toOpenAiMcpTools(mcpTools), tool_choice: "auto" } : basePayload; let response: unknown; try { response = await sendProviderPayload(payload); } catch (toolError) { if (!mcpTools.length) throw toolError; setToolNotice({ title: "Model chưa nhận MCP tools", detail: "Mở MCP Terminal trong Tools để gọi trực tiếp tool của server đã kết nối.", tone: "error" }); response = await sendProviderPayload(basePayload); } const calls = extractMcpToolCalls(response, mcpTools); if (calls.length) { if (calls.some(mcpCallMayChangeExternalData)) setToolNotice({ title: "Đang chạy MCP action", detail: "Theo cài đặt không hỏi lặp lại, tác vụ đã được AI đề nghị sẽ chạy và được ghi trong MCP Terminal.", tone: "success" }); await executeApprovedMcpTools({ payload, response, calls }); } else { appendAssistantResponse(response); if (mcpTools.length) setToolNotice({ title: "Model không trả function call", detail: "MCP Terminal trong Tools vẫn có thể gọi trực tiếp mọi tool đã kết nối.", tone: "error" }); } setAttachments([]); }
