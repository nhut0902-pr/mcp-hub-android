...
import { appendMcpToolResults, extractMcpToolCalls, functionNameForMcpTool, toOpenAiMcpTools } from "../lib/mcp-hub/mcp-chat";
...
describe("MCP chat tools", () => {
  ...
  it("chỉ nhận tool call khớp tool MCP đã tải", () => {
    ...
    expect(extractMcpToolCalls(response, [tool])).toEqual([{ id: "call-1", functionName: "mcp_github_main__list_issues", serverId: "github-main", serverName: "GitHub", toolName: "list_issues", argumentsValue: { repo: "owner/repo" } }]);
  });
  it("đưa kết quả tool vào message role tool để AI tổng hợp", () => {
    const call = { id: "call-1", functionName: functionNameForMcpTool(tool), serverId: tool.serverId, serverName: tool.serverName, toolName: tool.name, argumentsValue: {} };
